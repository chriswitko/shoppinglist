...
var CatalogItem =
  React.createClass({
    ...
    render: function() {
      ...
      var toolbarStyle = {
        display: (this._isAuthed()?'block':'none'),
      };
      ...
      return (
        <div ref="me" className="col-sm-3 col-xs-12 gridItem" style={itemStyle}>
          ...
          <div className="btn-group btn-group-xs mt10" style={toolbarStyle}>
            ...
            {this._isAuthed() ? <BtnFav isFaved={this.props.item.get('isFaved')} item={this.props.item} /> : null}
            {this._isAuthed() ? <BtnRemove item={this.props.item} idx={this.props.idx} /> : null}
          </div>
        </div>
        )
    }
